...
class drone_env_continuous_dynamic(gym.Env):
    def __init__(self):
        super(drone_env_continuous_dynamic, self).__init__()
    ...
    def getParentObjPos(self):
        self.MovingNames = ["ParentMovingFirstRow", "ParentMovingSecondRow", "ParentMovingThirdRow", "ParentMovingFourthRow"]
       
        for x in range(len(self.MovingNames)):
            self.posM = self.drone.simGetObjectPose(self.MovingNames[x])
            self.MovePos.append(self.posM.position.x_val)
        self.orien = self.posM.orientation
    ...
    def randomiseObjects(self):
        ...
        random.shuffle(self.MovePos)

        for x in range(len(self.MovePos)):
            print(self.MovingNames[x])
            print(self.MovePos[x])
            print(self.posM.position.y_val)
            print(self.posM.position.y_val)
            position = Vector3r(self.MovePos[x], self.posM.position.y_val, 0)
            pose = Pose(position, self.heading)
            #print(pose)
            self.drone.simSetObjectPose(self.MovingNames[x], pose, True)
    ...
    def getImageObs(self):
        ...
        #airsim.write_png(os.path.normpath(f'{image_path}/imageStacked{num}.png'), stacked_frames)
